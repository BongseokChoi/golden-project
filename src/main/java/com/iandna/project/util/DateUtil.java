...
public class DateUtil {
	...
	public static MapData getBabyMonth(MapData tData) {
		if(tData == null) {
			return tData;
		}
		String baby_birth;
		...
		int gapOfDay = 0;
		int baby_gbn_cd = 0;
		ArrayList<Integer> baby_month_arr = new ArrayList<Integer>();
		
		for(int i=3; i>0; i--) {
			baby_birth = tData.getString("baby_birth"+i);
			
			if( baby_birth != null && !baby_birth.isEmpty() && baby_birth.length() == 8) {
				gapOfDay = DateUtil.getDayDiff(baby_birth, DateUtil.getCurrentDate());
				
				if(gapOfDay < 0) {
					baby_gbn_cd = -1;
				}else {
					baby_gbn_cd = 1;
					baby_month = gapOfDay / 30 + 1;
					baby_month_arr.add(baby_month);
				}
			}else {
				baby_gbn_cd = -2;
			}
		}
		tData.set("baby_month_arr", baby_month_arr);
		tData.set("baby_gbn_cd", baby_gbn_cd);
		return tData;
	}

	public static String getNextDateTime(String date, int n) {
	...
	public static void main(String[] args) {
		...
		System.out.println(getNextMonth("202201", -1));
	}
}
